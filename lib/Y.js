...
Y.getTrackMp3Url = function (track, cb) {
    ...
    scrape.request(url, function(err, $) {
        ...
        scrape.request(fullUrl, function (err, $) {
            ...
            var regHostTagsFound = getRegionalHostTags($('*'))[0];
            var selectedHost = host;
            if(regHostTagsFound && regHostTagsFound.length) {
                selectedHost = regHostTagsFound.children[0].data;
            }
            ...
            var mp3Url = ('http://' + selectedHost + '/get-mp3/' + o + '/' +
                            ts + path + '?' +
